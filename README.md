...
`market-explanation` rotates eligible instruments by least recent attempt and skips accepted inputs already in the persisted cache. Refresh timestamps, transport telemetry and repeated identical impact-history samples do not create new work; changed evidence, observations and analytic values can. A queued or running instrument cannot acquire another simultaneous revision. In `visible`, the dashboard shows the last three accepted analyses, newest first, with previous answers expandable; pending or failed work preserves those answers. The additive `ai.marketExplanationHistory` field travels through the existing snapshot and `ai:update:v1` contracts, while `marketExplanations` remains available. See [market AI scheduling and history](docs/ai-market-history.md) for cache boundaries, retention and verification.
